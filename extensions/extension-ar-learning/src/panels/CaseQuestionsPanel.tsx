...
function CaseQuestionsPanel({
  servicesManager,
}: CaseQuestionsPanelProps) {
  ...
  async function refresh({ silent = false } = {}) {
    ...
    try {
      ...
      setAnswersByQuizKey(buildInitialAnswers(nextPayload.quizzes || [], nextPayload.responses || []));
  ...
  async function saveQuiz(quiz: QuizDefinition, status: 'draft' | 'submitted') {
    ...
    try {
      ...
      uiNotificationService.show({
        title: 'Case Questions',
        message: status === 'submitted' ? 'Answers submitted.' : 'Draft answers saved.',
        type: 'success',
        duration: 2500,
      });
  ...
  async function submitAllAndScore() {
    const requiredMissingQuiz = quizzes.find(quiz =>
      hasRequiredMissing(quiz, answersByQuizKey[quiz.quizKey] || {})
    );

    if (requiredMissingQuiz) {
      uiNotificationService.show({
        title: 'Case Questions',
        message: 'Please answer all required questions before submitting.',
        type: 'warning',
    ...
    try {
      for (const quiz of quizzes) {
        await saveViewerQuizResponseForActiveStudy({
          quizKey: quiz.quizKey,
          quizVersion: Number(quiz.quizVersion) || 1,
          status: 'submitted',
          answers: buildAnswerPayload(quiz, answersByQuizKey[quiz.quizKey] || {}),
        });
      }

      await submitViewerQuizScoreForActiveStudy();

      uiNotificationService.show({
        title: 'Case Questions',
        message: 'Answers submitted for scoring.',
        type: 'success',
        duration: 3000,
      });
      ...
      uiNotificationService.show({
        title: 'Case Questions',
        message: `Submit failed: ${error?.message || error}`,
        type: 'error',
        duration: 5000,
      });
    } finally {
  ...
  function renderQuestion(quiz: QuizDefinition, question: QuizQuestion) {
    const quizAnswers = answersByQuizKey[quiz.quizKey] || {};
    const value = quizAnswers[question.questionKey];
    ...
    <div className="flex h-full flex-col bg-black text-white">
      <div className="border-b border-gray-700 p-3">
        ...
        <div className="mt-1 text-xs text-gray-400">
          Viewer-based questions for this case.
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-3">
        {loading ? (
          ...
          <div className="space-y-4">
            {viewerQuizScore ? (
              <div className="rounded border border-green-700 bg-green-950/30 p-2 text-sm">
      ...
      {payload?.enabled && quizzes.length > 0 ? (
        <div className="border-t border-gray-700 p-3">
          <button
            ...
            onClick={submitAllAndScore}
          >
            {submitting ? 'Submitting…' : 'Submit Questions for Scoring'}
          </button>
        </div>
      ) : null}
    </div>
